...
const listProjectMessages = async (conversationId, tenantId, limit, cursor) => {
    ...
    const nextCursor = messages.length === safeLimit ? messages[messages.length - 1]._id : null;
...
const sendMessage = async (conversationId, projectId, tenantId, senderId, text) => {
    ...
    // Increment unread counts for all other participants (members + clients)
    const project = await Project.findById(projectId).select("members clients");
    if (project) {
        ...
        const otherUserIds = uniqueIds.filter(id => id !== senderId.toString());

        if (otherUserIds.length) {
            const bulkOps = otherUserIds.map(userId => ({
                updateOne: {
                    filter: { userId: new mongoose.Types.ObjectId(userId), projectId: projectObjectId },
                    update: {
                        $inc: { unreadCount: 1 },
                        $setOnInsert: { userId: new mongoose.Types.ObjectId(userId), projectId: projectObjectId }
                    },
                    upsert: true
                }
            }));
            await UserProjectMeta.bulkWrite(bulkOps);
        }
    }
...
const deleteMessage = async (messageId, actor, projectId, tenantId) => {
    ...
    if (conversation) {
        conversation.lastMessageAt = latestMessage ? latestMessage.createdAt : conversation.createdAt;
        await conversation.save();
    }
...
const markAsRead = async (conversationId, userId, tenantId) => {
    ...
    const result = await Message.updateMany(
        ...
        {
            $push: {
                readBy: { userId: userObjectId, seenAt: now }
            }
        }
    );

    // Reset precomputed unread count
    await UserProjectMeta.updateOne(
        { userId: userObjectId, projectId: conversation.projectId },
